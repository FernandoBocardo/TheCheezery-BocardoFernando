...
class ProductosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var adaptador: AdaptadorProductos = AdaptadorProductos(this, getListaProductos(tipo))
        listView.adapter = adaptador

    }

    fun getListaProductos(tipo: String?): ArrayList<Product>
    {
        var imagenTop: ImageView = findViewById(R.id.imagentop) as ImageView
        when(tipo)
        {
            "coldDrinks" -> {
                imagenTop.setImageResource(R.drawable.colddrinks)
                return coldDrinks
            }
            "hotDrinks" -> {
                imagenTop.setImageResource(R.drawable.hotdrinks)
                return hotDrinks
            }
            "salties" -> {
                imagenTop.setImageResource(R.drawable.hotdrinks)
